...
Helper kit for authors of [pGenie](https://pgenie.io) code generators. It layers fixtures and opt-in helpers on top of the [`gen-contract`](https://github.com/pgenie-io/gen-contract) schema, which generators pin and import separately.

`gen-sdk` itself carries no Haskell — all pGenie generators are written in Dhall. The canonical input model (`Project`) and the generator `module` constructor live in `gen-contract`, not here; a generator's `Deps/` needs both pins (see [`docs/generator-architecture.md`](docs/generator-architecture.md)).
